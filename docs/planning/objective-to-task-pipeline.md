...
The core insight: **agent success depends on context scope, not task "size"**.

| Complexity | File Span | Context Required | Agent Suitability |
|------------|-----------|------------------|-------------------|
| Trivial | 1 file | Just that file | Full auto |
| Easy | 1-2 files | Local module | Auto with review |
| Medium | 3-5 files | Feature area | Human monitors |
| Hard | 5+ files | Cross-cutting | Human does it |

This needs validation through research, but it's our working hypothesis.
...
**Question**: Does our file-span heuristic actually predict agent success?

To investigate:
- Review past Copilot PRs - categorize by file span, measure success rate
- Identify failure patterns - what context was missing?
- Define thresholds: at what file count does success drop off?
...
1. **File span predicts difficulty** - Need to verify with real data
2. **Island architecture limits context** - True for our repos, may not generalize
3. **Claude Code planning mode works** - Need to test with real objectives
4. **GitHub can handle our data model** - May need local storage
5. **One pipeline per repo is sufficient throughput** - Monitor and adjust
